...
<div class="input-group control-group {{ $classIncrement }} string @if ($required) required @endif {{ $model }}_{{ $field }}">
    <input class="form-control string @if ($required) required @endif @if ($errors->has($field)) is-invalid @endif" @if ($required) required="required" @endif autofocus="autofocus" type="text" name="{{ $field }}[]" value="{{ $value ?? '' }}" id="{{ $model }}_{{ $field }}" placeholder="{{ $placeholder ?? '' }}" />
...
<div class="hide mb-5 string @if ($required) required @endif {{ $model }}_{{ $field }}" id="{{ $idCloneKeywords }}">
    <div class="control-group control-keywords input-group mt-3" id="{{ $divRemove }}">
        <input class="form-control string @if ($required) required @endif @if ($errors->has($field)) is-invalid @endif" @if ($required) required="required" @endif autofocus="autofocus" type="text" name="{{ $field }}[]" value="{{ $value ?? '' }}" />
